...
function construirDocXml(xml, d){
  // ── Reemplazos quirúrgicos — preservan formato XML oficial al 100% ──

  // 1. N° de oficio (ej. " 1070.02  ")
  xml = xml.replace(
    /(<w:t[^>]*>) 1070\.02\s+(<\/w:t>)/,
    `$1 ${xmlEsc(d.numOficio||'1070.02')}  $2`
  );

  // 2. Fecha del oficio — va junto a "Bello," como párrafo siguiente
  xml = xml.replace(
    /(Bello,<\/w:t><\/w:r><\/w:p>)(<w:p [^>]+><w:pPr>[\s\S]*?<\/w:pPr><\/w:p>)/,
    `$1<w:p><w:pPr><w:spacing w:after="0" w:line="240" w:lineRule="auto"/><w:rPr><w:rFonts w:ascii="Arial" w:hAnsi="Arial" w:cs="Arial"/><w:sz w:val="18"/><w:szCs w:val="18"/></w:rPr></w:pPr><w:r><w:rPr><w:rFonts w:ascii="Arial" w:hAnsi="Arial" w:cs="Arial"/><w:sz w:val="18"/><w:szCs w:val="18"/></w:rPr><w:t>${xmlEsc(fmtL(d.fecha))}</w:t></w:r></w:p>`
  );

  // 3. Radicado en el asunto (color FF0000)
  xml = xml.replace(
    /(<w:color w:val="FF0000"[^>]*\/>(?:<w:sz[^>]*\/>)*(?:<w:szCs[^>]*\/>)*<\/w:rPr><w:t>)20252196416(<\/w:t>)/,
    `$1${xmlEsc(d.radicado)}$2`
  );

  // 4. Concepto + radicado en tabla (color EE0000) — ANTES del split global
  xml = xml.replace(
    /(<w:color w:val="EE0000"[^\/]*\/><w:sz[^\/]*\/><w:szCs[^\/]*\/><\/w:rPr><w:t>)EMBARGO TRANSITO 2023 20252196416(<\/w:t>)/,
    `$1${xmlEsc(d.concepto+' '+d.radicado)}$2`
  );

  // 5. Nombre contribuyente en tabla (color EE0000)
  xml = xml.replace(
    /(<w:color w:val="EE0000"[^\/]*\/><w:sz[^\/]*\/><w:szCs[^\/]*\/><\/w:rPr><w:t>)daniela higuita(<\/w:t>)/,
    `$1${xmlEsc(d.nombre)}$2`
  );

  // 6. Cédula en tabla (color EE0000, después del nombre)
  xml = xml.replace(
    /(<w:color w:val="EE0000"[^\/]*\/><w:sz[^\/]*\/><w:szCs[^\/]*\/><\/w:rPr><w:t>)1214735685(<\/w:t>)/,
    `$1${xmlEsc(d.cedula)}$2`
  );

  // 7. Todos los demás "20252196416" sueltos → radicado
  xml = xml.split('20252196416').join(xmlEsc(d.radicado));

  // 8. Fecha del auto (formato dd/mm/yyyy, subrayada en el XML)
  xml = xml.replace(
    /(<w:u w:val="single"[^\/]*\/><\/w:rPr><w:t>)19\/05\/2026(<\/w:t>)/,
    `$1${xmlEsc(d.fechaAuto)}$2`
  );

  // 9. Motivo del levantamiento
  xml = xml.replace(
    /(<w:t xml:space="preserve">)Facilidad o acuerdo de pago vigente (<\/w:t>)/,
    `$1${xmlEsc(d.motivo)} </w:t>`
  );

  // 10. Proyectó (fila de firmas): rellena Nombre, Firma y Fecha.
  //   - Nombre: nombre de quien proyectó (color EE0000, ya en plantilla).
  //   - Firma: el mismo nombre de quien proyectó.
  //   - Fecha: la fecha del auto (d.fechaAuto, formato dd/mm/yyyy).
  const _proy = xmlEsc(d.proyNombre||cfg.nombre||'');
  // Run reutilizable para Firma y Fecha (Arial 9pt, centrado)
  const _runFirma = (txt) =>
    '<w:r><w:rPr><w:rFonts w:ascii="Arial" w:hAnsi="Arial" w:cs="Arial"/>'+
    '<w:sz w:val="18"/><w:szCs w:val="18"/></w:rPr><w:t>'+txt+'</w:t></w:r>';
  // 10a. Nombre del proyectó (color EE0000)
  xml = xml.replace(
    /(<w:color w:val="EE0000"[^\/]*\/><w:sz[^\/]*\/><w:szCs[^\/]*\/><\/w:rPr><w:t>)Jeison Maya(<\/w:t>)/,
    `$1${_proy}$2`
  );
  // 10b. Firma (misma del proyectó) y Fecha (fecha del auto) en las dos
  //      celdas que siguen a la del nombre. Se anclan justo tras el cierre
  //      de la celda del nombre, insertando el contenido en los párrafos
  //      vacíos de las celdas Firma (1985) y Fecha (2126).
  xml = xml.replace(
    /(<\/w:r><\/w:p><\/w:tc>)(<w:tc><w:tcPr><w:tcW w:w="1985"[^>]*\/><\/w:tcPr><w:p [^>]*><w:pPr><w:jc w:val="center"\/><w:rPr><w:rFonts w:ascii="Arial" w:hAnsi="Arial" w:cs="Arial"\/><w:sz w:val="18"\/><w:szCs w:val="18"\/><\/w:rPr><\/w:pPr>)(<\/w:p><\/w:tc>)(<w:tc><w:tcPr><w:tcW w:w="2126"[^>]*\/><\/w:tcPr><w:p [^>]*><w:pPr><w:jc w:val="center"\/><w:rPr><w:rFonts w:ascii="Arial" w:hAnsi="Arial" w:cs="Arial"\/><w:sz w:val="18"\/><w:szCs w:val="18"\/><\/w:rPr><\/w:pPr>)(<\/w:p><\/w:tc>)/,
    (mm, cierreNombre, celdaFirma, finFirma, celdaFecha, finFecha) =>
      cierreNombre + celdaFirma + _runFirma(_proy) + finFirma +
      celdaFecha + _runFirma(xmlEsc(d.fechaAuto||'')) + finFecha
  );
...
function cargarConfigPanel(){
  ...
  if(b64stored && b64stored.length > 100){
    ...
    B64 = PLANTILLA_DEFAULT;
    actualizarEstadoPlantilla('Desembargo_bancario.docx (predeterminada — oficial Alcaldía)', PLANTILLA_DEFAULT.length);
  }
}
